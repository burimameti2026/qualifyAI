    ...
    IRequestHandler<ListRevenueAttributionQuery, IReadOnlyList<RevenueAttribution>>
{
    ...
    public async Task<WorkflowDesignerDto> Handle(GetWorkflowDesignerQuery q, CancellationToken ct) => new(
        await db.WorkflowNodes.AsNoTracking().Where(x => x.TenantId == q.TenantId && x.FlowId == q.FlowId).ToListAsync(ct),
        await db.WorkflowEdges.AsNoTracking().Where(x => x.TenantId == q.TenantId && x.FlowId == q.FlowId).ToListAsync(ct));
    ...
    public async Task<SalesPipelinesDto> Handle(GetSalesPipelinesQuery q, CancellationToken ct) => new(
        await db.Pipelines.AsNoTracking().Where(x => x.TenantId == q.TenantId).ToListAsync(ct),
        await db.PipelineStages.AsNoTracking().Where(x => x.TenantId == q.TenantId).OrderBy(x => x.SortOrder).ToListAsync(ct));
    ...
    public async Task<AnalyticsOverviewDto> Handle(GetAnalyticsOverviewQuery q, CancellationToken ct)
    {
        ...
        return new(
            leads.Count(x => x.Score >= 50),
            leads.Count(x => x.Score >= 80),
            opportunities.Where(x => x.Status == OpportunityStatus.Open).Sum(x => x.Amount),
            opportunities.Where(x => x.Status == OpportunityStatus.Won).Sum(x => x.Amount),
            await db.Conversations.CountAsync(x => x.TenantId == q.TenantId && x.AiEnabled, ct),
            await db.Tickets.CountAsync(x => x.TenantId == q.TenantId, ct));
    }
...
{
    public async Task<Company> Handle(CreateCompanyCommand c, CancellationToken ct) { c.Company.Id = Guid.NewGuid(); c.Company.TenantId = c.TenantId; db.Companys.Add(c.Company); await db.SaveChangesAsync(ct); return c.Company; }
    public async Task<Contact?> Handle(UpdateContactCommand c, CancellationToken ct) { var x = await db.Contacts.FirstOrDefaultAsync(v => v.Id == c.Id && v.TenantId == c.TenantId, ct); if (x is null) return null; x.FirstName=c.Contact.FirstName; x.LastName=c.Contact.LastName; x.Email=c.Contact.Email; x.Phone=c.Contact.Phone; x.LifecycleStage=c.Contact.LifecycleStage; x.CompanyId=c.Contact.CompanyId; await db.SaveChangesAsync(ct); return x; }
    public async Task<Opportunity?> Handle(UpdateOpportunityCommand c, CancellationToken ct) { var x=await db.Opportunitys.FirstOrDefaultAsync(v=>v.Id==c.Id&&v.TenantId==c.TenantId,ct); if(x is null)return null; x.Name=c.Opportunity.Name; x.Amount=c.Opportunity.Amount; x.Status=c.Opportunity.Status; x.PipelineStageId=c.Opportunity.PipelineStageId; x.ExpectedCloseUtc=c.Opportunity.ExpectedCloseUtc; await db.SaveChangesAsync(ct); return x; }
    public async Task<Opportunity?> Handle(MoveOpportunityStageCommand c, CancellationToken ct) { var x=await db.Opportunitys.FirstOrDefaultAsync(v=>v.Id==c.Id&&v.TenantId==c.TenantId,ct); if(x is null)return null; if(!await db.PipelineStages.AnyAsync(v=>v.Id==c.StageId&&v.TenantId==c.TenantId,ct)) throw new InvalidOperationException("Invalid pipeline stage"); x.PipelineStageId=c.StageId; db.CrmActivitys.Add(new CrmActivity{TenantId=c.TenantId,LeadId=x.LeadId,CompanyId=x.CompanyId,ContactId=x.ContactId,Type="pipeline",Subject="Opportunity moved",Body=c.StageId.ToString()}); await db.SaveChangesAsync(ct); return x; }
    public async Task<Conversation?> Handle(TakeoverConversationCommand c, CancellationToken ct) { var x=await db.Conversations.FirstOrDefaultAsync(v=>v.Id==c.ConversationId&&v.TenantId==c.TenantId,ct); if(x is null)return null; x.AiEnabled=false; if(c.UserId.HasValue)x.AssignedUserId=c.UserId; await db.SaveChangesAsync(ct); return x; }
    public async Task<Message?> Handle(AddConversationMessageCommand c, CancellationToken ct) { var conversation=await db.Conversations.FirstOrDefaultAsync(v=>v.Id==c.ConversationId&&v.TenantId==c.TenantId,ct); if(conversation is null)return null; var m=new Message{TenantId=c.TenantId,ConversationId=c.ConversationId,SenderType=string.IsNullOrWhiteSpace(c.SenderType)?"agent":c.SenderType,SenderUserId=c.UserId,Text=c.Text}; db.Messages.Add(m); conversation.LastMessageAtUtc=DateTime.UtcNow; await db.SaveChangesAsync(ct); return m; }
    public async Task<ConversationNote> Handle(AddConversationNoteCommand c, CancellationToken ct) { var n=new ConversationNote{TenantId=c.TenantId,ConversationId=c.ConversationId,UserId=c.UserId,Text=c.Text}; db.ConversationNotes.Add(n); await db.SaveChangesAsync(ct); return n; }
    public async Task<Conversation?> Handle(UpdateConversationCommand c, CancellationToken ct) { var x=await db.Conversations.FirstOrDefaultAsync(v=>v.Id==c.ConversationId&&v.TenantId==c.TenantId,ct); if(x is null)return null; if(Enum.TryParse<ConversationStatus>(c.Status,true,out var st))x.Status=st; if(c.AiEnabled.HasValue)x.AiEnabled=c.AiEnabled.Value; await db.SaveChangesAsync(ct); return x; }
    public async Task<Ticket?> Handle(UpdateTicketCommand c, CancellationToken ct) { var x=await db.Tickets.FirstOrDefaultAsync(v=>v.Id==c.Id&&v.TenantId==c.TenantId,ct); if(x is null)return null; x.Subject=c.Ticket.Subject; x.Description=c.Ticket.Description; x.Status=c.Ticket.Status; x.Priority=c.Ticket.Priority; x.AssignedUserId=c.Ticket.AssignedUserId; x.SlaPolicyId=c.Ticket.SlaPolicyId; if(x.Status==TicketStatus.Resolved||x.Status==TicketStatus.Closed)x.ResolvedAtUtc??=DateTime.UtcNow; db.TicketEvents.Add(new TicketEvent{TenantId=c.TenantId,TicketId=x.Id,Type="updated",DataJson=$"{{\"status\":\"{x.Status}\",\"priority\":\"{x.Priority}\"}}"}); await db.SaveChangesAsync(ct); return x; }
