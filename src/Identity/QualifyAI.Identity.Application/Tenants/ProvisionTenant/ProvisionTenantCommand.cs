...
{
    public async Task<ProvisionTenantResult> Handle(ProvisionTenantCommand request, CancellationToken cancellationToken)
    {
        ...
        return await strategy.ExecuteAsync(async () =>
        {
            ...
            return new ProvisionTenantResult(tenant.Id, license.Id, owner.Id, tenant.Status.ToString(), license.GetEffectiveStatus(DateTime.UtcNow).ToString(), license.Plan, license.MaxUsers, license.Modules.Where(x => x.Enabled).Select(x => x.Code).ToArray());
        });
    }
    ...
    private static void EnsureSucceeded(IdentityResult result)
    {
        ...
        throw new IdentityValidationException(result.Errors.GroupBy(x => string.IsNullOrWhiteSpace(x.Code) ? "identity" : x.Code).ToDictionary(x => x.Key, x => x.Select(y => y.Description).Distinct().ToArray()));
    }
}
