...
namespace LeadsAI.Identity.Domain.Tenants;
...
public sealed class Tenant : AggregateRoot
{
    ...
    {
        if (Status == TenantStatus.Closed) throw new InvalidOperationException("Closed tenants cannot be activated.");
    ...
    public void Suspend()
    {
        if (Status == TenantStatus.Closed) return;
    ...
    private static string NormalizeRequired(string value, string name) => string.IsNullOrWhiteSpace(value) ? throw new ArgumentException("Value is required.", name) : value.Trim();
    private static string NormalizeSlug(string value)
    {
        var slug = NormalizeRequired(value, nameof(value)).ToLowerInvariant();
        if (slug.Any(c => !(char.IsLetterOrDigit(c) || c == '-'))) throw new ArgumentException("Tenant slug may contain only letters, numbers and dashes.", nameof(value));
        return slug;
    }
}

public enum TenantStatus { Provisioning = 0, Active = 1, Suspended = 2, Closed = 3 }
