    ...
    IOutboxWriter outbox) : IAccountService
{
    public async Task<AccountResult> CreateUserAsync(CreateAccountRequest request, CancellationToken cancellationToken = default)
    {
        ...
        {
            await EnsureTenantCanAddUserAsync(request.TenantId, cancellationToken);
    ...
    public async Task<AccountResult?> GetUserAsync(Guid tenantId, Guid userId, CancellationToken cancellationToken = default) { var user = await users.Users.FirstOrDefaultAsync(x => x.TenantId == tenantId && x.Id == userId, cancellationToken); return user is null ? null : await MapAsync(user, cancellationToken); }
    public async Task<IReadOnlyList<AccountResult>> ListUsersAsync(Guid tenantId, CancellationToken cancellationToken = default) { var tenantUsers = await users.Users.Where(x => x.TenantId == tenantId).OrderBy(x => x.Email).ToListAsync(cancellationToken); var result = new List<AccountResult>(tenantUsers.Count); foreach (var user in tenantUsers) result.Add(await MapAsync(user, cancellationToken)); return result; }
    public Task SetRolesAsync(Guid tenantId, Guid userId, IReadOnlyCollection<string> roleNames, CancellationToken cancellationToken = default) => ExecuteInTransactionAsync(async () => { var user = await GetEntityAsync(tenantId, userId, cancellationToken); await SetRolesCoreAsync(user, roleNames, cancellationToken); await users.UpdateSecurityStampAsync(user); await QueueAccessChangedAsync(user, cancellationToken); await dbContext.SaveChangesAsync(cancellationToken); return 0; }, cancellationToken);
    public Task SetPermissionsAsync(Guid tenantId, Guid userId, IReadOnlyCollection<string> permissions, CancellationToken cancellationToken = default) => ExecuteInTransactionAsync(async () => { var user = await GetEntityAsync(tenantId, userId, cancellationToken); var existing = await dbContext.UserPermissions.Where(x => x.TenantId == tenantId && x.UserId == userId).ToListAsync(cancellationToken); dbContext.UserPermissions.RemoveRange(existing); dbContext.UserPermissions.AddRange(permissions.Where(x => !string.IsNullOrWhiteSpace(x)).Select(x => x.Trim()).Distinct(StringComparer.OrdinalIgnoreCase).Select(permission => new UserPermission { TenantId = tenantId, UserId = userId, Permission = permission })); await dbContext.SaveChangesAsync(cancellationToken); await users.UpdateSecurityStampAsync(user); await QueueAccessChangedAsync(user, cancellationToken); await dbContext.SaveChangesAsync(cancellationToken); return 0; }, cancellationToken);
    ...
    private async Task EnsureTenantCanAddUserAsync(Guid tenantId, CancellationToken cancellationToken)
    {
        var tenant = await dbContext.Tenants.AsNoTracking().FirstOrDefaultAsync(x => x.Id == tenantId, cancellationToken) ?? throw new IdentityValidationException("tenant", "Tenant not found.");
        if (tenant.Status != LeadsAI.Identity.Domain.Tenants.TenantStatus.Active) throw new IdentityValidationException("tenant", "Tenant is not active.");
        var license = await dbContext.Licenses.Include(x => x.Modules).FirstOrDefaultAsync(x => x.TenantId == tenantId, cancellationToken) ?? throw new IdentityValidationException("license", "Tenant does not have a license.");
        if (!license.IsUsable(DateTime.UtcNow)) throw new IdentityValidationException("license", $"Tenant license is {license.GetEffectiveStatus(DateTime.UtcNow)}.");
        var currentUsers = await users.Users.CountAsync(x => x.TenantId == tenantId, cancellationToken);
        if (currentUsers >= license.MaxUsers) throw new IdentityValidationException("maxUsers", $"User limit reached ({license.MaxUsers}).");
    }

    private Task SetActiveAsync(Guid tenantId, Guid userId, bool isActive, CancellationToken cancellationToken) => ExecuteInTransactionAsync(async () => { var user = await GetEntityAsync(tenantId, userId, cancellationToken); if (user.IsActive == isActive) return 0; user.IsActive = isActive; EnsureSucceeded(await users.UpdateAsync(user)); await users.UpdateSecurityStampAsync(user); await QueueAccessChangedAsync(user, cancellationToken); await dbContext.SaveChangesAsync(cancellationToken); return 0; }, cancellationToken);
    private async Task SetRolesCoreAsync(ApplicationUser user, IReadOnlyCollection<string> roleNames, CancellationToken cancellationToken) { var currentStorageNames = await users.GetRolesAsync(user); if (currentStorageNames.Count > 0) EnsureSucceeded(await users.RemoveFromRolesAsync(user, currentStorageNames)); var requestedRoles = roleNames.Where(x => !string.IsNullOrWhiteSpace(x)).Select(x => x.Trim()).Distinct(StringComparer.OrdinalIgnoreCase).ToArray(); foreach (var displayName in requestedRoles) { var storageName = TenantRoleNameCodec.ToStorageName(user.TenantId, displayName); var normalizedStorageName = storageName.ToUpperInvariant(); var role = await roles.Roles.FirstOrDefaultAsync(x => x.TenantId == user.TenantId && x.NormalizedName == normalizedStorageName, cancellationToken); if (role is null) { role = new ApplicationRole { Id = Guid.NewGuid(), TenantId = user.TenantId, Name = storageName, Description = displayName }; EnsureSucceeded(await roles.CreateAsync(role)); } EnsureSucceeded(await users.AddToRoleAsync(user, storageName)); } }
    ...
    private async Task<T> ExecuteInTransactionAsync<T>(Func<Task<T>> operation, CancellationToken cancellationToken) { var strategy = dbContext.Database.CreateExecutionStrategy(); return await strategy.ExecuteAsync(async () => { await using var transaction = await dbContext.Database.BeginTransactionAsync(cancellationToken); var result = await operation(); await transaction.CommitAsync(cancellationToken); return result; }); }
