...
public sealed class DemoSeeder(AppDbContext db, IPasswordService passwords)
{
    public async Task SeedAsync()
    {
        ...
        var tenant = new Tenant { Name="LeadsAI Demo", Slug="demo", PlanCode="business" };
        db.Tenants.Add(tenant);

        // Seed the entitlement projection directly, since this tenant is created locally
        // rather than through the normal TenantCreatedIntegrationEvent -> consumer flow that
        // populates TenantEntitlements for real tenants. Without this, the demo tenant would
        // have no row in TenantEntitlements and any entitlement/module checks against it
        // would fail as if the tenant had no license at all.
        db.TenantEntitlements.Add(new TenantEntitlementProjection
        {
            TenantId=tenant.Id,
            ...
            ExpiresAtUtc=null,
            Version=1,
            ModulesJson=System.Text.Json.JsonSerializer.Serialize(new[] { "ai", "automations", "sso" }),
            LimitsJson="{\"users\":50}",
        ...
        db.KnowledgeDocuments.AddRange(
            new KnowledgeDocument { TenantId=tenant.Id, KnowledgeBaseId=kb.Id, Title="LeadsAI Product Overview", Body="LeadsAI automates customer support, lead qualification, ticketing, workflows, CRM handoff and AI actions." },
            ...
            new KnowledgeDocument { TenantId=tenant.Id, KnowledgeBaseId=kb.Id, Title="CRM Integrations", Body="LeadsAI integrates through provider adapters, OAuth connections, mappings, sync jobs and webhooks." }
        );
        ...
        db.BrandingProfiles.Add(new BrandingProfile { TenantId=tenant.Id, ProductName="LeadsAI", PrimaryColor="#2563EB", AccentColor="#0B1220", SupportEmail="support@demo.local" });
        db.CustomDomains.Add(new CustomDomain { TenantId=tenant.Id, Host="support.demo.local", Status="verified", VerificationToken="demo" });
        db.SsoConfigurations.Add(new SsoConfiguration { TenantId=tenant.Id, ProviderType="saml", EntityId="urn:leadsai:demo", Enabled=false });
