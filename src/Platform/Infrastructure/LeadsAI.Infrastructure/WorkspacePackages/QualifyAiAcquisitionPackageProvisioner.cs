...
public sealed class QualifyAiAcquisitionPackageProvisioner(RealWorkspaceService workspace, AppDbContext db)
{
    public async Task<WorkspacePackageInstallResult> ProvisionAsync(Guid tenantId, CancellationToken ct = default)
    {
        var draft = await workspace.PrepareAsync(
            tenantId,
            new PrepareRealWorkspaceRequest("sales", "sales-acquisition", "LeadsAI Acquisition"),
            ct);
