        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            try
            {
                ...
                foreach (var group in due.GroupBy(x => x.TenantId))
                {
                    try
                    {
                        var modules = group.Select(x => x.ModuleCode).Distinct(StringComparer.OrdinalIgnoreCase).ToArray();
                        await orchestrator.ProvisionAsync(group.Key, modules, stoppingToken);
                        await lifecycle.ActivateAsync(new TenantLifecycleRequest(group.Key, modules), stoppingToken);
                    }
