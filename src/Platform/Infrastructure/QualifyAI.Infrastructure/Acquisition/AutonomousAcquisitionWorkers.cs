...
public sealed class AutonomousAcquisitionQueuedRunWorker(
    IServiceScopeFactory scopes,
    ILogger<AutonomousAcquisitionQueuedRunWorker> log) : BackgroundService
{
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            {
                log.LogError(ex, "Autonomous acquisition queue worker iteration failed");
            }
    ...
    private async Task ProcessDefaultDatabaseAsync(CancellationToken ct)
    {
    ...
    private async Task ProcessScopeAsync(IServiceProvider services, CancellationToken ct)
    {
        ...
        var ids = await db.AutonomousAcquisitionAgentRuns
            .Where(x => x.Status == AutonomousAgentRunStatus.Queued)
        ...
        foreach (var id in ids)
        {
            ...
            try
            {
                await orchestrator.ExecuteAsync(id, ct);
            }
            catch (Exception ex)
            {
...
public sealed class AutonomousAcquisitionSchedulerWorker(
    IServiceScopeFactory scopes,
    ILogger<AutonomousAcquisitionSchedulerWorker> log) : BackgroundService
{
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            {
                log.LogError(ex, "Autonomous acquisition scheduler iteration failed");
            }
    ...
    private async Task ScheduleDefaultDatabaseAsync(CancellationToken ct)
    {
        using var scope = scopes.CreateScope();
    ...
    private static async Task ScheduleScopeAsync(IServiceProvider services, CancellationToken ct)
    {
        ...
        var agents = await db.AutonomousAcquisitionAgents
            .Where(x => x.Status == AutonomousAgentStatus.Active)
            .ToListAsync(ct);

        foreach (var agent in agents)
        {
            var due = now.TimeOfDay >= agent.RunTimeUtc.ToTimeSpan();
            var already = agent.LastRunAtUtc.HasValue && DateOnly.FromDateTime(agent.LastRunAtUtc.Value) == today;
