...
public sealed class RealisticScenarioService(AppDbContext db)
{
    ...
    public async Task<ScenarioInstallResult> InstallAsync(Guid tenantId, CancellationToken ct = default)
    {
        ...
        if(icp is null)
        {
            icp=new IcpProfile
            {
                ...
                CountriesCsv="Germany, France, Italy",
                MinimumEmployees=20,
                MaximumEmployees=1000,
                IntentKeywordsCsv="freight tender, warehouse expansion, fleet growth, delivery delays",
                CriteriaJson="{\"requiredSignals\":1,\"minimumFitScore\":70}",
                Active=true,
                LastDiscoveryAtUtc=now
            };
        ...
        if(prospects.Count==0)
        {
            foreach(var seed in prospectSeeds)
            {
                var prospect = new Prospect
                {
                    ...
                    Source="presentation-scenario",
                    Priority="A",
                    ContactReadiness="Demo ready",
                    SuggestedBuyer=seed.JobTitle,
                    PainHypothesis=seed.Evidence,
                    Offer="FusionFleet operational platform pilot",
                    SourceUrl=$"https://signals.example/{seed.Domain}",
                    VerificationStatus="Presentation scenario",
                    OutreachStatus="Ready for approval",
                    DatasetOrigin="FusionFleet presentation scenario"
                };
            ...
            foreach(var prospect in prospects)
                if(!await db.OutreachMessages.AnyAsync(x => x.TenantId==tenantId&&x.CampaignId==campaign.Id&&x.ProspectId==prospect.Id&&x.CampaignStepId==firstStep.Id, ct))
                    db.OutreachMessages.Add(new OutreachMessage
                    {
                        ...
                        Channel="email",
                        Subject=$"{prospect.CompanyName}: reduce dispatch and delivery exceptions",
    ...
    private async Task EnsureAutomationsAsync(Guid tenantId, Guid discoveryIcpId, DateTime now, CancellationToken ct)
    {
        ...
        var discoveryActions = JsonSerializer.Serialize(new object[]
        {
