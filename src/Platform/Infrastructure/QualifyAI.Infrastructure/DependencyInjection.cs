...
public static class DependencyInjection
{
    public static IServiceCollection AddBusinessInfrastructure(this IServiceCollection services, IConfiguration configuration, bool allowDevelopmentModelDrift = false)
    {
        ...
        services.AddScoped<IBusinessUnitOfWork, BusinessUnitOfWork>(); services.AddScoped<ICrmRepository, CrmRepository>(); services.AddScoped<ISupportRepository, SupportRepository>(); services.AddScoped<IKnowledgeAiRepository, KnowledgeAiRepository>(); services.AddScoped<IWorkflowAutomationRepository, WorkflowAutomationRepository>(); services.AddScoped<ITenantEntitlementRepository, TenantEntitlementRepository>(); services.AddScoped<IdentityEntitlementInboxProcessor>(); services.AddScoped<IGoldenPipelineProvisioner, GoldenPipelineProvisioner>(); services.AddScoped<IModuleProvisioner, GoldenPipelineModuleProvisioner>(); services.AddScoped<IModuleRegistry, ModuleRegistry>(); services.AddScoped<IModuleProvisioningOrchestrator, ModuleProvisioningOrchestrator>(); services.AddScoped<ITenantLifecycleOrchestrator, TenantLifecycleOrchestrator>(); services.AddScoped<ILicenseChangeOrchestrator, LicenseChangeOrchestrator>(); services.AddHostedService<ModuleProvisioningRetryWorker>();
