...
public static class DependencyInjection
{
 ...
 {
  services.AddDbContext<AppDbContext>(o=>{o.UseSqlServer(configuration.GetConnectionString("DefaultConnection"),sql=>sql.EnableRetryOnFailure());if(allowDevelopmentModelDrift)o.ConfigureWarnings(w=>w.Ignore(RelationalEventId.PendingModelChangesWarning));});
  services.AddSingleton<ITenantLifecycleEventStore,TenantLifecycleEventStore>();services.AddSingleton<ITenantAlertService,TenantAlertService>();services.AddScoped<ITenantLifecycleHealthService,TenantLifecycleHealthService>();services.AddSingleton(new BillingLifecyclePolicy());services.AddSingleton<IBillingLifecycleEngine,BillingLifecycleEngine>();services.AddScoped<IUsageMeter,PersistentUsageMeter>();services.AddScoped<IBillingQuotaEnforcer,BillingQuotaEnforcer>();services.AddScoped<IBillingAlertSink,PersistentBillingAlertSink>();services.AddScoped<IBillingEventProcessor,BillingEventProcessor>();services.AddScoped<IBillingProvider,StripeBillingProvider>();services.AddScoped<BillingProviderRegistry>();
  ...
  services.AddScoped<ITenantContext,TenantContext>();services.AddScoped<IPasswordService,PasswordService>();services.AddScoped<IKnowledgeRetriever,SqlKnowledgeRetriever>();services.AddScoped<IAiProvider,LocalAiProvider>();services.AddScoped<IAiTool,CreateLeadTool>();services.AddScoped<IAiTool,CreateTicketTool>();services.AddScoped<IAiTool,SearchKnowledgeTool>();services.AddScoped<IAiToolRegistry,AiToolRegistry>();services.AddScoped<IIntegrationRegistry,IntegrationRegistry>();services.AddScoped<SalesAutomationService>();services.AddScoped<DemoSeeder>();services.AddScoped<CampaignExecutionService>();services.AddScoped<ProspectReplyProcessingService>();services.AddScoped<ProspectDiscoveryService>();services.AddSingleton<IAutonomousAcquisitionTemplateRegistry,AutonomousAcquisitionTemplateRegistry>();services.AddScoped<IAutonomousAcquisitionRunOrchestrator,AutonomousAcquisitionRunOrchestrator>();services.AddHttpClient<SerpApiProspectDiscoveryProvider>(c=>{c.BaseAddress=new Uri("https://serpapi.com/");c.Timeout=TimeSpan.FromSeconds(60);});services.AddScoped<IProspectDiscoveryProvider>(sp=>sp.GetRequiredService<SerpApiProspectDiscoveryProvider>());services.AddScoped<AutomationActionExecutor>();services.AddScoped<RealisticScenarioService>();services.AddScoped<IEmailDeliveryProvider,SmtpEmailProvider>();services.AddHttpClient<BrevoEmailProvider>(c=>{c.BaseAddress=new Uri("https://api.brevo.com/v3/");c.Timeout=TimeSpan.FromSeconds(60);});services.AddScoped<IEmailDeliveryProvider>(sp=>sp.GetRequiredService<BrevoEmailProvider>());services.AddHttpClient<SendGridEmailProvider>();services.AddScoped<IEmailDeliveryProvider>(sp=>sp.GetRequiredService<SendGridEmailProvider>());services.AddScoped<EmailDeliveryService>();return services;
 }
}
