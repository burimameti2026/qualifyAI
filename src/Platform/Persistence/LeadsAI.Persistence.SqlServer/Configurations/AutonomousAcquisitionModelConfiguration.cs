...
public static class AutonomousAcquisitionModelConfiguration
{
 ...
 {
  builder.Entity<CampaignContainer>(e=>{e.ToTable("CampaignContainers");e.HasKey(x=>x.Id);e.Property(x=>x.Name).HasMaxLength(200).IsRequired();e.Property(x=>x.PackageCode).HasMaxLength(128).IsRequired();e.Property(x=>x.PackageVersion).HasMaxLength(32).IsRequired();e.Property(x=>x.ConfigurationJson).IsRequired();e.HasIndex(x=>new{x.TenantId,x.CampaignId});e.HasIndex(x=>new{x.TenantId,x.AgentId}).IsUnique();});
  ...
  builder.Entity<AutonomousAcquisitionTask>(e=>{e.ToTable("AutonomousAcquisitionTasks");e.HasKey(x=>x.Id);e.Property(x=>x.Type).HasMaxLength(64).IsRequired();e.Property(x=>x.Name).HasMaxLength(200).IsRequired();e.Property(x=>x.ConfigurationJson).IsRequired();e.Property(x=>x.ResultJson).IsRequired();e.Property(x=>x.Error).HasMaxLength(4000);e.HasIndex(x=>new{x.TenantId,x.AgentId,x.RunId,x.Sequence}).IsUnique().HasFilter("[RunId] IS NOT NULL");e.HasIndex(x=>new{x.TenantId,x.Status});});
  builder.Entity<AutonomousAcquisitionAgentRun>(e=>{e.ToTable("AutonomousAcquisitionAgentRuns");e.HasKey(x=>x.Id);e.Property(x=>x.Query).HasMaxLength(2000);e.Property(x=>x.Error).HasMaxLength(4000);e.HasIndex(x=>new{x.AgentId,x.CampaignId,x.ScheduledAtUtc});e.HasIndex(x=>new{x.TenantId,x.Status});});
 }
}
