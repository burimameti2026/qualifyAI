    ...
    IConsumer<TenantLicenseChangedIntegrationEvent>
{
    ...
    {
        // SQL Server retry-on-failure cannot be combined with a user transaction
        // unless the entire transaction is executed through the configured
        // execution strategy. This is important for MassTransit consumers because
        // transient SQL failures must replay the complete inbox + mutation unit.
        ...
        await strategy.ExecuteAsync(async () =>
        {
            ...
            if (await db.InboxMessages.AnyAsync(
                    x => x.Id == eventId && x.Consumer == ConsumerName, ct))
            {
