...
public sealed class AutomationSchedulerWorker(IServiceScopeFactory scopeFactory, IOptions<AutomationSchedulerOptions> options, ILogger<AutomationSchedulerWorker> logger) : BackgroundService
{
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        if (!options.Value.Enabled) { logger.LogInformation("Automation scheduler is disabled globally."); return; }
    ...
    private async Task RunDueAsync(CancellationToken ct)
    {
        ...
        var enabledTenantIds = await scope.ServiceProvider.GetRequiredService<TenantWorkerRuntime>().EnabledTenantIdsAsync(TenantWorkerKeys.AutomationScheduler, ct);
        var rules = await db.AutomationRules.Where(x => x.Active && x.Trigger == "schedule.weekday" && enabledTenantIds.Contains(x.TenantId)).OrderBy(x => x.TenantId).ThenBy(x => x.Id).ToListAsync(ct);
        foreach (var rule in rules)
        {
            ...
            var run = AutomationRun.Create(rule.TenantId, rule.Id, triggerData); db.AutomationRuns.Add(run); run.Start(); await db.SaveChangesAsync(ct);
