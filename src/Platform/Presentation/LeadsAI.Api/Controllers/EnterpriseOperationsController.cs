...
public sealed class EnterpriseOperationsController(ITenantContext tenant, AppDbContext db) : ControllerBase
{
    ...
    {
        var id = TenantId;
        return Ok(new { facilities = await db.Facilities.CountAsync(x => x.TenantId == id && x.IsActive, ct), customers = await db.CustomerAccounts.CountAsync(x => x.TenantId == id && x.IsActive, ct), orders = await db.SalesOrders.CountAsync(x => x.TenantId == id, ct), pendingOrders = await db.SalesOrders.CountAsync(x => x.TenantId == id && x.Status == LeadsAI.Domain.OrderStatus.Submitted, ct), stockItems = await db.StockBalances.CountAsync(x => x.TenantId == id, ct), movementsInTransit = await db.StockMovements.CountAsync(x => x.TenantId == id && x.Status == StockMovementStatus.InTransit, ct), shipments = await db.Shipments.CountAsync(x => x.TenantId == id && x.Status != LeadsAI.Domain.ShipmentStatus.Delivered && x.Status != LeadsAI.Domain.ShipmentStatus.Cancelled, ct), unpaidPayments = await db.Payments.CountAsync(x => x.TenantId == id && x.Status != LeadsAI.Domain.PaymentStatus.Paid && x.Status != LeadsAI.Domain.PaymentStatus.Cancelled && x.Status != LeadsAI.Domain.PaymentStatus.Refunded, ct) });
    }
    ...
    { var order = await db.SalesOrders.FirstOrDefaultAsync(x => x.TenantId == TenantId && x.Id == id, ct); if (order is null) return NotFound(); if ((int)status < (int)order.Status && status != LeadsAI.Domain.OrderStatus.Cancelled) return BadRequest(new { detail = "Order status cannot move backwards." }); order.Status = status; order.UpdatedAtUtc = DateTime.UtcNow; await db.SaveChangesAsync(ct); return Ok(order); }
    ...
    { var entity = await db.Shipments.FirstOrDefaultAsync(x => x.TenantId == TenantId && x.Id == id, ct); if (entity is null) return NotFound(); if ((int)status < (int)entity.Status && status != LeadsAI.Domain.ShipmentStatus.Cancelled) return BadRequest(new { detail = "Shipment status cannot move backwards." }); entity.Status = status; entity.UpdatedAtUtc = DateTime.UtcNow; if (status == LeadsAI.Domain.ShipmentStatus.Delivered) entity.DeliveredAtUtc = DateTime.UtcNow; await db.SaveChangesAsync(ct); return Ok(entity); }
    ...
    { var entity = await db.Payments.FirstOrDefaultAsync(x => x.TenantId == TenantId && x.Id == id, ct); if (entity is null) return NotFound(); entity.Status = status; entity.UpdatedAtUtc = DateTime.UtcNow; if (status == LeadsAI.Domain.PaymentStatus.Paid) entity.PaidAtUtc = DateTime.UtcNow; await db.SaveChangesAsync(ct); return Ok(entity); }
}
