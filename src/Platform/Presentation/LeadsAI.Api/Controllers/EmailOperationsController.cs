    ...
    IConfiguration configuration) : ControllerBase
{
    ...
    public async Task<IActionResult> BrevoWebhook([FromBody] JsonElement payload, [FromHeader(Name = "X-LeadsAI-Webhook-Token")] string? token, [FromQuery] string? accessToken, CancellationToken ct)
    {
    ...
    public async Task<IActionResult> SendGridWebhook(
        ...
        [FromHeader(Name = "X-LeadsAI-Webhook-Token")] string? token,
        ...
        CancellationToken ct)
    {
        ...
        foreach (var item in events)
        {
            ...
            var correlationId = ReadString(item, "leadsai_message_id");
    ...
    {
        var correlationId = HeaderValue(headers, "X-LeadsAI-Message-Id");
        ...
        CancellationToken ct)
    {
        ...
        return await provider.SendAsync(new EmailEnvelope(
            email, string.IsNullOrWhiteSpace(name) ? "LeadsAI" : name,
            email, string.IsNullOrWhiteSpace(name) ? "Sender" : name,
            "Confirm your LeadsAI sending identity",
            $"<p>Enter this verification code in LeadsAI:</p><p><strong>{token}</strong></p><p>If you did not request this, ignore this email.</p>",
            $"Enter this verification code in LeadsAI: {token}",
            null), ct);
    }
