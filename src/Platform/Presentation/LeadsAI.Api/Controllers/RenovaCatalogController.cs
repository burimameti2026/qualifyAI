...
using LeadsAI.Domain.Core.Portal;
...
public sealed class RenovaCatalogController(ITenantContext tenant, AppDbContext db) : ControllerBase
{
    ...
    public async Task<IActionResult> UpdateProduct(Guid id, CatalogProduct request, CancellationToken ct)
    {
        ...
        if (!await db.ProductCategories.AnyAsync(x => x.Id == request.ProductCategoryId && x.TenantId == TenantId && x.IsActive, ct)) return BadRequest(new { code = "category_not_found" });
        if (await db.CatalogProducts.AnyAsync(x => x.TenantId == TenantId && x.Code == request.Code.Trim() && x.Id != id, ct)) return Conflict(new { code = "product_code_exists" });
    ...
    {
        language = NormalizeLanguage(language); var product = await db.CatalogProducts.SingleOrDefaultAsync(x => x.Id == id && x.TenantId == TenantId && x.IsActive, ct); if (product is null) return NotFound();
    ...
    {
        var product = await db.CatalogProducts.SingleOrDefaultAsync(x => x.Id == id && x.TenantId == TenantId && x.IsActive, ct); if (product is null) return NotFound(); request ??= new PublishProductRequest(null);
        ...
        else { publication.Status = "Published"; publication.IsVisible = true; if (!string.IsNullOrWhiteSpace(request.Slug)) publication.Slug = Slug(request.Slug, id); publication.Version++; publication.PublishedAt = DateTimeOffset.UtcNow; publication.UpdatedAt = DateTimeOffset.UtcNow; }
    ...
    [HttpGet("products/{id:guid}/markets")]
    public Task<List<TargetMarket>> Markets(Guid id, CancellationToken ct) => db.TargetMarkets.AsNoTracking().Where(x => x.TenantId == TenantId && x.CatalogProductId == id && x.IsActive).OrderBy(x => x.CountryName).ToListAsync(ct);
    ...
    {
        if (!await db.CatalogProducts.AnyAsync(x => x.Id == id && x.TenantId == TenantId && x.IsActive, ct)) return NotFound();
        item.Id = Guid.NewGuid(); item.TenantId = TenantId; item.CatalogProductId = id; item.CountryCode = item.CountryCode.Trim().ToUpperInvariant(); item.DefaultLanguage = NormalizeLanguage(item.DefaultLanguage);
        var existing = await db.TargetMarkets.SingleOrDefaultAsync(x => x.TenantId == TenantId && x.CatalogProductId == id && x.CountryCode == item.CountryCode, ct);
        if (existing is not null) return Ok(existing);
        db.TargetMarkets.Add(item); await db.SaveChangesAsync(ct); return Ok(item);
    }

    [HttpGet("promotion-plans")]
    public Task<List<ProductPromotionPlan>> PromotionPlans(CancellationToken ct) => db.ProductPromotionPlans.AsNoTracking().Where(x => x.TenantId == TenantId).OrderByDescending(x => x.CreatedAt).ToListAsync(ct);
    ...
    public async Task<IActionResult> CreatePlan(ProductPromotionPlan item, CancellationToken ct)
    {
        ...
        item.Id = Guid.NewGuid(); item.TenantId = TenantId; item.CreatedAt = DateTimeOffset.UtcNow; item.CampaignLanguage = NormalizeLanguage(item.CampaignLanguage); item.Status = string.IsNullOrWhiteSpace(item.Status) ? "Draft" : item.Status; db.ProductPromotionPlans.Add(item); await db.SaveChangesAsync(ct); return Ok(item);
    }

    [HttpPost("promotion-plans/{id:guid}/activate")]
    public async Task<IActionResult> ActivatePlan(Guid id, CancellationToken ct)
    {
        var plan = await db.ProductPromotionPlans.SingleOrDefaultAsync(x => x.Id == id && x.TenantId == TenantId, ct); if (plan is null) return NotFound(); plan.Status = "Active"; plan.ActivatedAt = DateTimeOffset.UtcNow; await db.SaveChangesAsync(ct); return Ok(plan);
    }
