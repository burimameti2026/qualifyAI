    ...
    ProspectDiscoveryService discovery) : ControllerBase
{
    ...
    {
        var campaign = new Campaign { TenantId=TenantId, TargetListId=input.TargetListId, OfferId=input.OfferId, Name=input.Name.Trim(), Goal=input.Goal, SenderName=input.SenderName.Trim(), SenderEmail=input.SenderEmail.Trim(), StartsAtUtc=input.StartsAtUtc };
        db.Campaigns.Add(campaign);
        db.CampaignSteps.AddRange(input.Steps.OrderBy(x => x.StepNumber).Select(x => new CampaignStep { TenantId=TenantId, CampaignId=campaign.Id, StepNumber=x.StepNumber, DelayHours=x.DelayHours, Channel=x.Channel, SubjectTemplate=x.SubjectTemplate, BodyTemplate=x.BodyTemplate, RulesJson=JsonSerializer.Serialize(new CampaignStepRules(x.Qualification, x.MinimumScore, x.Industry, x.Countries, x.CompanySizeMin, x.CompanySizeMax, x.ContactRoles, x.StopOnReply)) }));
