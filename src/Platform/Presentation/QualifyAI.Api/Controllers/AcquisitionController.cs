    ...
    ProspectDiscoveryService discovery) : ControllerBase
{
    ...
    public async Task<IActionResult> VerifyDiscoveryProvider(string name, CancellationToken ct)
    {
        ...
        {
            return NotFound(new { code = "discovery_provider_not_found", detail = exception.Message });
        }
    }
    ...
    [HttpGet("templates")]
    [RequirePermission(QualifyAiPermissions.CrmRead)]
    public Task<List<OutreachTemplate>> Templates(CancellationToken ct) =>
        db.OutreachTemplates.AsNoTracking().Where(x => x.TenantId == TenantId && x.IsActive).OrderBy(x => x.Name).ToListAsync(ct);
    ...
    public async Task<IActionResult> CreateTemplate(OutreachTemplateInput input, CancellationToken ct)
    {
        ...
        if (await db.OutreachTemplates.AnyAsync(x => x.TenantId == TenantId && x.Name == name, ct))
            return Conflict(new { detail = "A template with this name already exists for this tenant." });
    ...
    public async Task<IActionResult> UpdateTemplate(Guid id, OutreachTemplateInput input, CancellationToken ct)
    {
        ...
        if (await db.OutreachTemplates.AnyAsync(x => x.TenantId == TenantId && x.Id != id && x.Name == name, ct))
            return Conflict(new { detail = "A template with this name already exists for this tenant." });
        template.Name = name; template.Description = input.Description?.Trim() ?? string.Empty; template.SubjectTemplate = input.SubjectTemplate.Trim(); template.BodyTemplate = input.BodyTemplate.Trim(); template.Touch();
        await db.SaveChangesAsync(ct);
        return Ok(template);
    }
    ...
    public async Task<IActionResult> DeleteTemplate(Guid id, CancellationToken ct)
    {
        var template = await db.OutreachTemplates.FirstOrDefaultAsync(x => x.TenantId == TenantId && x.Id == id, ct);
        if (template is null) return NotFound();
        template.IsActive = false; template.Touch();
        await db.SaveChangesAsync(ct);
        return NoContent();
    }
    ...
    public async Task<IActionResult> Campaigns(CancellationToken ct)
    {
        ...
        var campaigns = await db.Campaigns.AsNoTracking().Where(x => x.TenantId==tenantId)
            ...
            .Select(campaign => new
            {
                ...
                steps = db.CampaignSteps.Where(step => step.TenantId == tenantId && step.CampaignId == campaign.Id).OrderBy(step => step.StepNumber).Select(step => new { step.StepNumber, step.DelayHours, step.Channel, step.SubjectTemplate, step.BodyTemplate, templateId = step.TemplateId, templateName = db.OutreachTemplates.Where(template => template.Id == step.TemplateId).Select(template => template.Name).FirstOrDefault() }).ToList(),
